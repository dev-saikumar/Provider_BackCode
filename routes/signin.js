...
route.post("/",async (req,res)=>{
    try {
        ...
        var response= await Model.findOne({fuid:req.body.fuid}).lean();
        if(response==null)
...
try {
    ...
    user=user.toJSON();
        if(user.fuid=="0"){
        var response= await Model.findOneAndUpdate({"uid":req.body.uid},{$set:{fuid: req.body.fuid}},{new:true,useFindAndModify:true});
        res.status(200).send(response).end();
        }
        else{
